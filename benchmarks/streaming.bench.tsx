...
const createStreamingChunks = (content: string, chunkCount: number): string[] => {
  const chunks: string[] = [];
...
const createCharacterChunks = (content: string, charsPerChunk: number): string[] => {
  const chunks: string[] = [];
...
describe("Streaming Performance: Small Updates", () => {
  describe("Character-by-character (50 chars/update) - Medium Document", () => {
    ...
    bench(
      "Streamdown (streaming with parseIncompleteMarkdown)",
      () => {
    ...
    bench(
      "Streamdown (streaming without parseIncompleteMarkdown)",
      () => {
        const { rerender } = render(
