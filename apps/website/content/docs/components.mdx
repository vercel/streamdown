...
When streaming markdown, custom components can detect if their block is still being streamed using the `useIsBlockIncomplete` hook. This is useful for expensive-to-render components where you want to show a loading state until the block is complete.
