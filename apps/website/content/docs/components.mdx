...
<Callout type="warn">
  ...
  If you need to preserve the default appearance and behavior, import `defaultComponents` and render the built-in component inside your override. This keeps default styles and attributes like `data-streamdown` in sync with Streamdown updates.

  Prefer this over re-applying Tailwind class strings by hand. For style-only tweaks, you can also use [CSS selectors with `data-streamdown` attributes](/docs/styling#global-css-targeting).
</Callout>
...
export default function Page({ markdown }: { markdown: string }) {
  return (
    <Streamdown
      components={{
        // ❌ Loses default spacing, font styles, and data-streamdown
        h2: ({ children }) => (
          <h2 className="text-blue-500">{children}</h2>
        ),
        // ✅ Composes on top of the built-in component
        h2: (props) => (
          <DefaultH2 {...props} className={`${props.className ?? ""} text-blue-500`} />
        ),
      }}
    >
      {markdown}
...
You can also wrap defaults for conditional behavior:

```tsx title="app/page.tsx"
import { createElement, type ComponentProps } from "react";
import { Streamdown, defaultComponents, type ExtraProps } from "streamdown";

function AOverride(props: ComponentProps<"a"> & ExtraProps) {
  if (props.href?.startsWith("/internal")) {
    return <a {...props} className="text-emerald-600" />;
  }
...
export default function Page({ markdown }: { markdown: string }) {
  return (
    <Streamdown components={{ a: AOverride }}>
      {markdown}
    </Streamdown>
  );
}
