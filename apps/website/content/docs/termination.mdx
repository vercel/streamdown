...
description: Learn how Streamdown handles incomplete Markdown syntax during AI streaming with remedy.
---

One of Streamdown's most powerful features is its ability to intelligently parse and style incomplete Markdown blocks using the [remedy](https://www.npmjs.com/package/remedy) package. This feature, called **unterminated block parsing**, ensures that your streaming content looks polished even before the AI finishes its response.

## About Remedy

Remedy is a lightweight, standalone preprocessor that completes incomplete Markdown syntax. Streamdown integrates remedy by default, but you can also use it independently in your own projects. See the [remedy documentation](https://www.npmjs.com/package/remedy) for standalone usage.
...
Remedy (Streamdown's preprocessing layer) analyzes the incoming Markdown and identifies common patterns that might be incomplete. When detected, it automatically adds the closing syntax so the content renders correctly, then seamlessly updates when the actual closing syntax arrives.
...
Unterminated block parsing is enabled by default in Streamdown. You can disable the remedy preprocessor if needed:
...
### Using Remedy Standalone

You can use remedy independently in your own markdown rendering pipeline:
...
See the [remedy package](https://www.npmjs.com/package/remedy) for more details on standalone usage.

## Smart Behavior

Remedy includes intelligent rules to avoid false positives:
...
Remedy is highly optimized for streaming scenarios:
