...
Streamdown uses a hybrid approach for syntax highlighting: common languages and themes are bundled locally, while additional ones load on-demand from a CDN. This guide shows you how to configure Streamdown for environments without internet access.
...
In offline environments, CDN loading fails and code blocks fall back to plain text (for languages) or the default GitHub themes (for themes).

## Configure Self-Hosted Assets

To run Streamdown offline, you need to:

1. Host Shiki language and theme files locally
2. Configure your application to serve them
3. Tell Streamdown where to find them

### Step 1: Download Shiki Files
...
### Step 2: Serve Shiki Files

Configure your framework to serve the Shiki language and theme files as static assets.
...
Serve the Shiki files as static assets:
...
Pass the `cdnUrl` prop to the Streamdown component to specify the base path for Shiki files:
...
The `cdnUrl` prop sets the base path. Streamdown will automatically append `/langs/{language}.mjs` for languages and `/themes/{theme}.mjs` for themes.
