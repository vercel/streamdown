...
- `<pre>` — preformatted / fenced code blocks (CommonMark always emits `pre > code`, so a closed, Shiki-highlighted block stays un-split). An unclosed fence is the exception: `pre` animates until the closing fence arrives, then it is skipped again.
- `<svg>` — vector graphics
...
Completed fenced blocks, syntax-highlighted code, math equations, and diagrams render without animation spans. While a fence is still open, its code fades in with the prose. Inline code always does.
