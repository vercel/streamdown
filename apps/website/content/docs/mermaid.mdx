...
Mermaid is loaded on-demand from a CDN only when a diagram is rendered, keeping your initial bundle size small. By default, Mermaid loads from `https://streamdown.ai/cdn`. See [Self-Hosting CDN](/docs/self-hosting-cdn) for self-hosting options.
