...
Mermaid is loaded on-demand from a CDN only when a diagram is rendered, keeping your initial bundle size small. By default, Mermaid loads from `https://www.streamdown.ai/cdn`. See [Self-Hosting CDN](/docs/self-hosting-cdn) for self-hosting options.
...
```tsx
<Streamdown mermaid={{ config: { theme: "forest" } }}>{markdown}</Streamdown>
```
...
```tsx
<Streamdown controls={{ mermaid: false }}>{markdown}</Streamdown>
```
