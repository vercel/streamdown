...
Mermaid diagrams are expensive to render. During streaming, you don't want to re-render the diagram on every chunk—that would be slow and cause flickering. Use the `useIsBlockIncomplete` hook to wait until the code block is complete before rendering:

```tsx
import { Streamdown, useIsBlockIncomplete } from "streamdown";
import { mermaid } from "@streamdown/mermaid";
